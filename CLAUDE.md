...
**Never surface a raw `/detail` field as a capability flag.** Firmware simply omits fields that don't apply to a model, so an absent value reads as "unknown", not "no". `hasMatlStation` is the known case: it is AD5X-only, and a Creator 5 Pro leaves it out of `/detail` entirely (verified on firmware 1.9.4) while reporting a fully populated `matlStationInfo` with four loaded slots. `FFMachineInfo.has_matl_station` is therefore **derived** in `MachineInfoParser.from_detail` (flag `is True` OR `slotCnt > 0` OR non-empty `slotInfos`) and always concrete; the raw value stays on `FFPrinterDetail`. Apply the same rule to any new capability: derive it from the data the capability actually produces, and cover it with a fixture built from a real payload — the Creator 5 fixtures originally had no material station, which is why this went unnoticed through v1.3.1.
