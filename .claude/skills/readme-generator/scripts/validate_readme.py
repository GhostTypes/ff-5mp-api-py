...
class ReadmeValidator:
    ...
    def check_centered_structure(self):
        ...
        if center_divs < 2:
            self.warnings.append(
                f"⚠️ Only {center_divs} centered section(s) found - most content should be centered"
            )
    ...
    def check_no_standalone_bullets(self):
        ...
        for i, line in enumerate(lines, 1):
            ...
            if "|" in line and not in_table:
                ...
                and line.strip()
                and "|" not in line
                and not line.strip().startswith("</div>")
            ):
                in_table = False
            ...
            if not in_table and re.match(r"^\s*[-*]\s+", line):
                self.errors.append(
                    f"❌ Standalone bullet list found at line {i}: {line.strip()[:50]}"
                )
    ...
    def check_code_blocks_not_centered(self):
        ...
        if re.search(pattern, self.content, re.DOTALL):
            ...
            # For now, just warn
            self.warnings.append(
                "⚠️ Code block might be inside center div - verify it's in a table cell"
            )
